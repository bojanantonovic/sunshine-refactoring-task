...
public class OrderLog {
	...
	public static void logAddPancake(Order order, String description, List<PancakeRecipe> pancakes) {
		long pancakesInOrder = getPancakesInOrder(order, pancakes);
	...
	private static long getPancakesInOrder(final Order order, final List<PancakeRecipe> pancakes) {
		return pancakes.stream() //
				.filter(pancakeRecipe -> pancakeRecipe.getOrderId().equals(order.getId())) //
				.count();
	}
}
